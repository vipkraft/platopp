...
begin
  ...
  begin
   ...
   ZQuery1.SQL.add('SELECT a.id,a.name as pname FROM av_users AS a WHERE a.del=0 AND a.category='+inttostr(tusr) +' ORDER BY a.name ASC;');
  end
  else
 ...
 for n:=1 to ZQuery1.RecordCount do
   begin
      combobox2.Items.Add(ZQuery1.FieldByName('pname').asString+' -- ' + ZQuery1.FieldByName('point_id').asString);
      ZQuery1.Next;
     end;
  ...
  for n:=0 to slist.Count-1 do
   begin
    stmp := '';
     ...
     ////разделитель
     //IF utf8pos('[f45]',FMemo1.Lines[n])>0 then bold:=1;
     // begin
