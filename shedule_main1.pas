...
uses
  Classes, SysUtils, ZDataset, ZConnection, FileUtil, Forms,
  Controls, Graphics, Dialogs, ExtCtrls, StdCtrls, Buttons, ComCtrls, Grids,StrUtils,TicketTypes, LCLProc;
...
procedure TForm15.UpdateGrid(filter_type:byte; stroka:string);
 ...
 begin
  ...
  //filter_shedule=2;//фильтр РАСПИСАНИЙ муниципальных через Автовокзал
  //filter_shedule=3;//фильтр РАСПИСАНИЙ муниципальных через Автостанцию №1
  //filter_shedule=4;//фильтр РАСПИСАНИЙ муниципальных через Автостанцию №2
  ...
  //  Form15.ZQuery1.SQL.add('LEFT JOIN av_route AS b ON a.id_route=b.id AND b.del=0 ');
  //  end;
  //If filter_shedule=1 then
  //  begin
  //label2.Caption:='Расписания межобластные и межгосударственные';
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=2 or b.type_route=3) ');
  //  end;
  //If filter_shedule=2 then
  //begin
  //label2.Caption:='Расписания муниципальные через Автовокзал';
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=0 or b.type_route=1) ');
  //ZQuery1.SQL.add('Join av_shedule_sostav AS w ON w.id_point=815 AND a.id=w.id_shedule AND w.del=0 ');
  //end;
   //If filter_shedule=3 then
  //begin
  //label2.Caption:='Расписания муниципальные через Автостанцию №1';
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=0 or b.type_route=1) ');
  //ZQuery1.SQL.add('Join av_shedule_sostav AS w ON w.id_point=816 AND a.id=w.id_shedule AND w.del=0 ');
  //end;
    //If filter_shedule=4 then
  //begin
  //label2.Caption:='Расписания муниципальные через Автостанцию №2';
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=0 or b.type_route=1) ');
  //ZQuery1.SQL.add('Join av_shedule_sostav AS w ON w.id_point=814 AND a.id=w.id_shedule AND w.del=0 ');
  //end;
    //If filter_shedule=5 then
  //begin
  //label2.Caption:='Расписания муниципальные другие';
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=0 or b.type_route=1) ');
  //end;
  //Form15.ZQuery1.SQL.add('Left JOIN av_spr_locality AS e ON b.id_nas1=e.id AND e.del=0 ');
  //Form15.ZQuery1.SQL.add('Left JOIN av_spr_locality AS c ON b.id_nas2=c.id AND c.del=0 ');
  //Form15.ZQuery1.SQL.add('Left JOIN av_spr_locality AS d ON b.id_nas3=d.id AND d.del=0 ');
//||---------------------------------------------------------
//||=====================  фильтр расписаний
//||
...
begin
 ...
 Form15.ZQuery1.SQL.add('WHERE a.del<>1 ');
  ...
  stmp:='  z.type_route=1 ';
    ...
    If filter_shedule=4 then
  begin
  label2.Caption:='Расписания межмуниципальные и прогородные через '+ConnectINI[14];
  //Form15.ZQuery1.SQL.add('JOIN av_route AS b ON a.id_route=b.id AND b.del=0 AND (b.type_route=0 or b.type_route=1) ');
  ...
  stmp:='  (z.type_route=0 or z.type_route=1) ';
    ...
    If filter_shedule=5 then
  begin
  label2.Caption:='Расписания межмуниципальные и прогородные НЕ через '+ConnectINI[14];
 ...
 If Form15.CheckBox9.Checked then
  ...
  If sort_direction=1 then
           ZQuery1.SQL.add(' ASC, z.dateactive ASC;')
      else ZQuery1.SQL.add(' DESC, z.dateactive ASC; ');
...
begin
  ...
  with sender as TStringGrid do
   begin
    if (Cells[aCol,aRow]='0') then  cells[aCol,aRow]:='1' else cells[aCol,aRow]:='0';
  ...
  If aCol=8 then
   ...
   If (Cells[aCol,aRow]='0')  then
     begin
     form15.ZQuery1.SQL.Clear;
      form15.ZQuery1.SQL.add('UPDATE av_shedule_fio SET del=1,createdate=now(),id_user='+inttostr(id_user)+' WHERE id_shedule='+ Cells[0,aRow] +' and del=0;');
      try
      form15.ZQuery1.open;
      except
      form15.Zconnection1.disconnect;
   ...
   begin
    ...
    form15.ZQuery1.SQL.add(inttostr(id_user)+','+ Cells[0,aRow] +',now(),'+inttostr(id_user)+ ',0);');
    form15.ZQuery1.open;
   end;
    end;
  ...
  If (aCol=10) or (aCol=11) then
   ...
   If (Cells[aCol,aRow]='0')  then
     begin
     form15.ZQuery1.SQL.Clear;
      form15.ZQuery1.SQL.add('UPDATE av_shedule_remote_sale_permition SET del=1,createdate=now(),id_user='+inttostr(id_user)+' WHERE id_shedule='+ Cells[0,aRow] +' and del=0;');
      //showmessage(ZQuery1.SQL.Text);//$
      try
      form15.ZQuery1.open;
      except
   ...
   begin
    ...
    form15.ZQuery1.SQL.add(inttostr(id_user)+','+ Cells[0,aRow] +',now(),'+inttostr(id_user)+','+Cells[10,aRow]+','+Cells[11,aRow]+',0);');
    //showmessage(ZQuery1.SQL.Text);//$
    form15.ZQuery1.open;
   end;
    end;
  ...
  If aCol=12 then
   ...
   If (Cells[aCol,aRow]='0')  then
     begin
     form15.ZQuery1.SQL.Clear;
      form15.ZQuery1.SQL.add('UPDATE av_shedule_tel SET del=1,createdate=now(),id_user='+inttostr(id_user)+' WHERE id_shedule='+ Cells[0,aRow] +' and del=0;');
      try
      form15.ZQuery1.open;
      except
      form15.Zconnection1.disconnect;
