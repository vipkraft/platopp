...
begin
  ...
  if form22.GroupBox3.Enabled=true then
    begin
  ...
  if flag_edit_sostav=1 then
       begin
           ...
           if (trim(form22.Edit2.text)='') or
              ...
              begin
                showmessagealt('Заполнены не все обязательные поля с данными !'+#13+'Сохранение невозможно !');
                exit;
              end;
  ...
  if flag_edit_sostav=2 then
       begin
           ...
           if (trim(form22.Edit2.text)='') or
              (trim(form22.Edit1.text)='') then
              begin
                showmessagealt('Внесите наименование остановочного пункта !'+#13+'Сохранение невозможно !');
                exit;
              end;
           IF (form16.StringGrid1.Row>1) AND (form22.SpinEdit9.Value=0) then
               begin
                showmessagealt('Нулевое расстояние между пунктами !'+#13+'Сохранение невозможно !');
                exit;
              end;
...
begin
    With Form22 do
    begin
    // Если добавляем или редактируем не первый остановочный пункт
         ...
         If not(Connect2(Zconnection1, flagProfile)) then
          ...
          //Делаем запрос к справочнику расстояний
           ...
           Form22.ZQuery1.open;
             ...
             showmessagealt('Выполнение команды SQL - ОШИБКА !'+#13+Form22.ZQuery1.SQL.Text);
             Form22.ZQuery1.Close;
             Form22.Zconnection1.disconnect;
             exit;
           end;
    ...
    if flag_edit_sostav=2 then
       begin
          ...
          GroupBox2.Enabled:= true;
            //если последний пункт ставим флаг последнего пункта в составе
            if (form16.StringGrid1.Row=form16.StringGrid1.RowCount-1) and (form16.StringGrid1.Rowcount>2)  then
              begin
          ...
          if (form16.StringGrid1.Row=1) or (form16.StringGrid1.Rowcount<2) then
          begin
        ...
        // Заполняем форму значениями
            edit5.text:=trim(form16.StringGrid1.Cells[0,form16.StringGrid1.row-1]);
            edit3.text:=trim(form16.StringGrid1.Cells[1,form16.StringGrid1.row-1]);
            Edit2.text:=m_sostav[form16.StringGrid1.row,0];  //код остановочного пункта
            Edit1.text:=m_sostav[form16.StringGrid1.row,1];  //Наименование остановочного пункта
            if m_sostav[form16.StringGrid1.row,2]='1' then  CheckBox1.Checked:=true else  CheckBox1.Checked:=false; //Формирующийся
            try
             SpinEdit2.value:=strtoint(m_sostav[form16.StringGrid1.row ,3]); //Платформа отправления
             SpinEdit1.value:=strtoint(m_sostav[form16.StringGrid1.row ,4]); //Платформа прибытия
             SpinEdit3.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,5]),1,2));  //Время отправления
             SpinEdit4.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,5]),4,2));  //Время отправления
          except
             on exception: EConvertError do
             begin
           ...
           try
             SpinEdit7.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,7]),1,2));  //Время стоянки
             SpinEdit8.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,7]),4,2));  //Время стоянки
             SpinEdit10.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,9]),1,2)); //Время в пути
             SpinEdit11.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,9]),4,2)); //Время в пути
             SpinEdit9.value:=strtofloat(m_sostav[form16.StringGrid1.row ,8]);                  //Расстояние
          except
             on exception: EConvertError do
             begin
             ...
             Edit7.text:=trim(m_sostav[form16.StringGrid1.row,12]);     //Смещение часового пояса
            if  not(trim( Edit7.text)='0') then
                  check_param()
            else
                 begin
                   try
                  SpinEdit5.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,6]),1,2));  //Время прибытия
                   except
                      on exception: EConvertError do
                      begin
            ...
            try
              SpinEdit6.value:=strtoint(copy(trim(m_sostav[form16.StringGrid1.row ,6]),4,2));  //Время прибытия
           except
             on exception: EConvertError do
             begin
            ...
            if m_sostav[form16.StringGrid1.row,13]='1' then  CheckBox2.Checked:=true else  CheckBox2.Checked:=false; //Точка возврата
       end;

    //************** если открыт в режиме добавления ***********************
    if flag_edit_sostav=1 then
       begin
         GroupBox1.Enabled:= true;
            // Если первый пункт в составе
         ...
         // Если следующий пункт то сразу километраж
           if form16.StringGrid1.RowCount>1 then
              begin
               ...
               // Рисуем информацию об ост.пунктах
               // Пред. ост. пункт
                edit5.text:=trim(form16.StringGrid1.Cells[0,form16.StringGrid1.rowcount-1]);
                edit3.text:=trim(form16.StringGrid1.Cells[1,form16.StringGrid1.rowcount-1]);
              end;
