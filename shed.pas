...
var
  ...
  Y2, M2, D2:Word;
  Y3, M3, D3:Word;
  tekDate, maxDate,firstdate : TDateTime;
  Check:boolean=true;
...
begin
 ...
 //формируем основной массив
   ...
   If Check then GetSezon(Form16.DateEdit3.Date,tekdate+DaysInaMonth(nyear,nmonth)-1,sdate,arSezon);
showmas(arSezon);
  ...
  for m:=0 to DaysInAMonth(nyear, nmonth)-1 do
    begin
      ...
      If ((tekdate+m)<form16.DateEdit1.date) OR ((tekdate+m)>form16.DateEdit2.date) then
         masday[length(masday)-1,5] := 1
         else masday[length(masday)-1,5] := 0;
     If not Check then continue;
     If (tekdate+m) < Form16.DateEdit3.Date then continue;
...
begin
  ...
  DecodeDate(form16.DateEdit1.Date,Y1, M1, D1);
  DecodeDate(form16.DateEdit2.Date,Y2, M2, D2);
  //ОТВАЛ, если год вне диапазона
  If (nyear<Y1) AND (nyear>Y2) then exit;
  //ОТВАЛ, если месяц вне диапазона
  If (nmonth<M1) AND (nmonth>M2) then exit;
 ...
 //showmessage('sdate='+copy(sdate,16,4));
 tekdate:= EncodeDate(nyear,nmonth,1);
 ...
 // Подключаемся к серверу
   ...
   //masday[n,3] - номер недели месяца
    ...
    FormGr.ZQuery1.SQL.add('Select case (cast(EXTRACT(WEEK FROM DATE '+quotedstr(datetostr(tekdate+m))+') as integer) %4) when 0 ');
    FormGr.ZQuery1.SQL.add('then 4 else (cast (EXTRACT(WEEK FROM DATE '+quotedstr(datetostr(tekdate+m))+') as integer) %4) END as num;');
     try
      FormGr.ZQuery1.open;
    except
    ...
    //определяем флаг выхода
    FormGr.ZQuery1.SQL.Clear;
    FormGr.ZQuery1.SQL.add('SELECT getsezon('+quotedstr(sdate)+','+quotedstr(datetostr(form16.DateEdit3.date))+','+quotedstr(datetostr(tekdate+m))+') as res;');
  ...
  with Sender as TStringGrid, Canvas do
   begin
    ...
    {if (gdSelected in aState) then
     ...
     if (aRow>0) and not(trim(Cells[aCol, aRow])='')  and (masday[strtoint(Cells[aCol, aRow])-1,5]=0) then
         begin
          if masday[strtoint(Cells[aCol, aRow])-1,1]=1 then
             begin
     ...
     if (aRow>0) and not(trim(Cells[aCol, aRow])='')  and (masday[strtoint(Cells[aCol, aRow])-1,5]=1) and (MonthOfTheYear(Form16.DateEdit3.Date)=formGr.ComboBox1.ItemIndex+1) then
         begin
            pen.Width:=4;
            pen.Color:=clRed;
     ...
     if (aRow>0) and not(trim(Cells[aCol, aRow])='') then
        begin
         Font.Size:=12;
         Font.Color := clGreen;
         if (strtoint(Cells[aCol, aRow]) mod 2)>0 then TextOut(aRect.Left + 10, aRect.Top+30, 'Нечетное');
         if (strtoint(Cells[aCol, aRow]) mod 2)=0 then TextOut(aRect.Left + 10, aRect.Top+30, 'Четное');
        end;
     ...
     if (aRow>0) and not(trim(Cells[aCol, aRow])='') then
        begin
         Font.Size:=12;
         Font.Color := clGray;
         TextOut(aRect.Left + 10, aRect.Top+45, inttostr(masday[strtoint(Cells[aCol, aRow])-1,3])+'-Неделя');
    ...
    { if (aRow>0) and not(trim(Cells[aCol, aRow])='') then
        begin
         Font.Size:=8;
         Font.Color := clGray;
         TextOut(aRect.Left + 10, aRect.Top+60, 'Ч\Н: '+masday[strtoint(Cells[aCol, aRow])-1,2]+' Нед: '+masday[strtoint(Cells[aCol, aRow])-1,3]+' ДНед: '+masday[strtoint(Cells[aCol, aRow])-1,4]);
        end;}
