...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, Buttons, Grids, ComCtrls, Spin, EditBtn,
  ActnList, StrUtils, FMemo, ticket, FMemo_Type, MouseAndKeyInput,
  kmemo, KEditCommon, KGraphics;
  //LCLIntf, LResources,
  //Variants, KFunctions, KControls, KDialogs;
type
  ...
  TformTT = class(TForm)
    ...
    procedure FMemo1Change(Sender: TObject);
    procedure FMemo1Click(Sender: TObject);
    procedure FMemo1KeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure FMemo2Change(Sender: TObject);
    procedure FMemo2KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure ToBParaLeftClick(Sender: TObject);
    procedure ToolButton6Click(Sender: TObject);
    procedure ToolButton8Click(Sender: TObject);
    procedure UpdateMemo(); //вывод грида шаблона
    ...
    procedure MemoInsert(str:string); //вставить текст с мемо с сохранением форматирования
    procedure StrAlign(alin:string); //выравнивание строки в мемо
    procedure UpdateKMemo(); //вывод шаблона в мемо
  private
...
//procedure TFormTT.TextStyleChanged(Sender: TObject);
//var
//  SelAvail: Boolean;
...
//  SelAvail := Kmemo1.SelAvail;
//  SelEnd := Kmemo1.SelEnd;
//  if SelAvail then
//    Kmemo1.SelectionTextStyle := FTextStyle
//  else if Kmemo1.GetNearestWordIndexes(SelEnd, False, StartIndex, EndIndex) and (StartIndex < SelEnd) and (SelEnd < EndIndex) then
...
//    Kmemo1.SetRangeTextStyle(StartIndex, EndIndex, FTextStyle)
//  else
//    Kmemo1.NewTextStyle := FTextStyle;
//end;
//
//procedure TFormTT.ParaStyleChanged(Sender: TObject; AReasons: TKMemoUpdateReasons);
//begin
//  Kmemo1.SelectionParaStyle := FParaStyle;
//end;

procedure TFormTT.ChangeActionState(AAction: TObject; AState: Boolean);
begin
...
procedure TFOrmTT.MemoInsert(str:string); //вставить текст с мемо с сохранением форматирования
var
  fp:TFontDesc;
 ss,sl :integer;
 prefix: string='';
...
begin
  ...
  If PageControl1.ActivePageIndex=0 then
    ...
    FMemo1.SetFocus;
    ss:=FMemo1.SelStart;
    sl:=FMemo1.CaretPos.x;
    FMemo1.GetFormat(fp);
    If trim(FMemo1.SelText)='' then
     stmp := utf8copy(FMemo1.Lines[FMemo1.CaretPos.Y],1,sl)+str+Utf8copy(FMemo1.Lines[FMemo1.CaretPos.Y],sl+1,Utf8length(FMemo1.Lines[FMemo1.CaretPos.Y])-sl)
    else stmp := StringReplace(FMemo1.Lines[FMemo1.CaretPos.Y],FMemo1.SelText,str, [rfReplaceAll, rfIgnoreCase]);
    FMemo1.Lines.Delete(FMemo1.CaretPos.Y);
    FMemo1.Lines.Insert(FMemo1.CaretPos.Y,stmp);
   //выделяем всю строку
   // Ставим в начало строки
   FMemo1.SetFocus;
   pos.x:=0;
   sl:= FMemo1.Lines.IndexOf(stmp);
   pos.y:=sl;
   FMemo1.CaretPos:=pos;
   //восстанавливаем форматирование
   FMemo1.SelLength := UTF8Length(FMemo1.Lines[sl]);
   fp.BackColor:=clWhite;
   FMemo1.SetFormat(fp, FMemo1.SelStart, FMemo1.SelLength);
   FMemo1.SelLength:=0;
   FMemo1.SelStart:=ss;
  ...
  If PageControl1.ActivePageIndex=1 then
    ...
    FMemo2.SetFocus;
    ss:=FMemo2.SelStart;
    sl:=FMemo2.CaretPos.x;
    FMemo2.GetFormat(fp);
    If trim(FMemo2.SelText)='' then
     stmp := utf8copy(FMemo2.Lines[FMemo2.CaretPos.Y],1,sl)+str+Utf8copy(FMemo2.Lines[FMemo2.CaretPos.Y],sl+1,Utf8length(FMemo2.Lines[FMemo2.CaretPos.Y])-sl)
    else stmp := StringReplace(FMemo2.Lines[FMemo2.CaretPos.Y],FMemo2.SelText,str, [rfReplaceAll, rfIgnoreCase]);
    FMemo2.Lines.Delete(FMemo2.CaretPos.Y);
    FMemo2.Lines.Insert(FMemo2.CaretPos.Y,stmp);
   //выделяем всю строку
   // Ставим в начало строки
   FMemo2.SetFocus;
   pos.x:=0;
   sl:= FMemo2.Lines.IndexOf(stmp);
   pos.y:=sl;
   FMemo2.CaretPos:=pos;
   //восстанавливаем форматирование
   FMemo2.SelLength := UTF8Length(FMemo2.Lines[sl]);
   fp.BackColor:=clWhite;
   FMemo2.SetFormat(fp, FMemo2.SelStart, FMemo2.SelLength);
   FMemo2.SelLength:=0;
   FMemo2.SelStart:=ss;
...
begin
 ...
 if ZQuery1.RecordCount=1 then
     begin
       FMemo2.Text:=ZQuery1.FieldByName('stext').AsString;
...
procedure TformTT.FMemo1Change(Sender: TObject);
begin
  fl_change:=true;
end;

procedure TformTT.FMemo1Click(Sender: TObject);
var
  fp:TFontDesc;
begin
  formTT.FMemo1.GetFormat(fp);
  formTT.StatusBar1.Canvas.Clear;
  formTT.StatusBar1.Canvas.Font.Size:=10;
  formTT.StatusBar1.Canvas.TextOut(5,0,'Текущая позиция. Символ:'+inttostr(formTT.FMemo1.CaretPos.x)+' Строка:'+inttostr(formTT.FMemo1.CaretPos.Y)+' Шрифт='+inttostr(fp.Size));
end;

procedure TformTT.FMemo1KeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  fp:TFontDesc;
begin
  formTT.FMemo1.GetFormat(fp);
...
procedure TformTT.FMemo2Change(Sender: TObject);
begin
  fl_dop_change:=true;
end;

procedure TformTT.FMemo2KeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
  fl_dop_change:=true;
end;
...
begin
 ...
 //If n>20 then showmessage(stmp);
  ...
  //FMemo1.Lines.Delete(FMemo1.CaretPos.Y);
  FMemo1.Lines.add(stmp);
  pos.x:=0;
  pos.y:=fmemo1.Lines.IndexOf(stmp);
  FMemo1.CaretPos:=pos;
  FMemo1.SetFormat(fp,FMemo1.SelStart-FMemo1.CaretPos.X,UTF8Length(FMemo1.Lines[FMemo1.CaretPos.Y]));
  end;
...
begin
  ...
  //убираем переменные из текста
     ...
     stmp := StringReplace(stmp,'[fright]','',[rfReplaceAll, rfIgnoreCase]);
         ...
         TB := AddTextBlock(stmp);
         PA := AddParagraph;

    //выравнивание слева
     IF utf8pos('[fleft]',slist[n])>0 then PA.ParaStyle.HAlign := halLeft;
     //выравнивание справа
     IF utf8pos('[fright]',slist[n])>0 then PA.ParaStyle.HAlign := halRight;
     //выравнивание по центру
     IF utf8pos('[fcenter]',slist[n])>0 then PA.ParaStyle.HAlign := halCenter;
  //шрифт № 6
   IF utf8pos('[f6]',slist[n])>0 then
     begin
...
//шрифт № 1
  IF utf8pos('[f1]',slist[n])>0 then
  begin
    TB.TextStyle.Font.Size:=12;
    //fp.Style :=fpNormal;
  end;
//шрифт № 4
 IF utf8pos('[f4]',slist[n])>0 then
  begin
    TB.TextStyle.Font.Size:=14;
...
begin
  ...
  if (FMemo1.Lines.Count<2) or (FMemo1.CaretPos.Y=0) then exit;
  //сохранить новую позицию курсора
  position1.x:=FMemo1.CaretPos.x;
  position1.y:=FMemo1.CaretPos.y-1;
  //Меняем местами строчки грида
  FMemo1.Lines.Exchange(FMemo1.CaretPos.Y,FMemo1.CaretPos.Y-1);
  FMemo1.CaretPos := position1;
  //восстанавливаем форматирование
  FMemo1.SetFormat(fp,FMemo1.SelStart-FMemo1.CaretPos.X,UTF8Length(FMemo1.Lines[FMemo1.CaretPos.Y]));
  fl_change := true; //флаг внесения изменений
  end;
  ...
  begin
    ...
    FMemo2.GetFormat(fp);
  // Если нет пунктов или первый то ничего не делаем
  if (FMemo2.Lines.Count<2) or (FMemo2.CaretPos.Y=0) then exit;
  //сохранить новую позицию курсора
  position1.x:=FMemo2.CaretPos.x;
  position1.y:=FMemo2.CaretPos.y-1;
  //Меняем местами строчки грида
  FMemo2.Lines.Exchange(FMemo2.CaretPos.Y,FMemo2.CaretPos.Y-1);
  FMemo2.CaretPos := position1;
  //восстанавливаем форматирование
  FMemo2.SetFormat(fp,FMemo2.SelStart-FMemo2.CaretPos.X,UTF8Length(FMemo2.Lines[FMemo2.CaretPos.Y]));
  fl_dop_change := true; //флаг внесения изменений
  end;
  end;
end;
...
begin
  ...
  begin
    ...
    FMemo1.GetFormat(fp);
   // Если нет пунктов или первый то ничего не делаем
    if (FMemo1.Lines.Count<2) or (FMemo1.CaretPos.Y=FMemo1.Lines.Count-1) then exit;
    //сохранить новую позицию курсора
    position2.x:=FMemo1.CaretPos.x;
    position2.y:=FMemo1.CaretPos.y+1;
    //Меняем местами строчки грида
    FMemo1.Lines.Exchange(FMemo1.CaretPos.Y,FMemo1.CaretPos.Y+1);
    FMemo1.CaretPos := position2;
    //восстанавливаем форматирование
    FMemo1.SetFormat(fp,FMemo1.SelStart-FMemo1.CaretPos.X,UTF8Length(FMemo1.Lines[FMemo1.CaretPos.Y]));
    fl_change := true; //флаг внесения изменений
    end;
  ...
  begin
    ...
    FMemo2.GetFormat(fp);
   // Если нет пунктов или первый то ничего не делаем
    if (FMemo2.Lines.Count<2) or (FMemo2.CaretPos.Y=FMemo2.Lines.Count-1) then exit;
    //сохранить новую позицию курсора
    position2.x:=FMemo2.CaretPos.x;
    position2.y:=FMemo2.CaretPos.y+1;
    //Меняем местами строчки грида
    FMemo2.Lines.Exchange(FMemo2.CaretPos.Y,FMemo2.CaretPos.Y+1);
    FMemo2.CaretPos := position2;
    //восстанавливаем форматирование
    FMemo2.SetFormat(fp,FMemo2.SelStart-FMemo2.CaretPos.X,UTF8Length(FMemo2.Lines[FMemo2.CaretPos.Y]));
    fl_dop_change := true; //флаг внесения изменений
    end;
...
procedure TformTT.ACParaLeftExecute(Sender: TObject);
begin
    if not FActionStateChanging then
    FParaStyle.HAlign := halLeft;
end;

procedure TformTT.ACParaCenterExecute(Sender: TObject);
begin
     if not FActionStateChanging then
    FParaStyle.HAlign := halCenter;
end;

procedure TformTT.ACParaCenterUpdate(Sender: TObject);
begin
  ChangeActionState(Sender, FParaStyle.HAlign = halCenter);
end;

procedure TformTT.ACParaLeftUpdate(Sender: TObject);
begin
     ChangeActionState(Sender, FParaStyle.HAlign = halLeft);
end;

procedure TformTT.ACParaRightExecute(Sender: TObject);
begin
   if not FActionStateChanging then
    FParaStyle.HAlign := halRight;
end;

procedure TformTT.ACParaRightUpdate(Sender: TObject);
begin
...
begin
  ...
  If trim(FMemo1.Lines.Text)='' then
    begin
  ...
  //ищем конец шаблона чека
   for n:=0 to FMemo1.Lines.Count-1 do
   begin
    stmp := trim(FMemo1.Lines[n]);
 ...
 begin
  ...
  for n:=0 to FMemo1.Lines.Count-1 do
   begin
    // Ставим в середину строки
     pos.x:=Utf8length(FMemo1.Lines[n]) div 2;
     pos.y:=n;
     FMemo1.CaretPos:=pos;
     FMemo1.GetFormat(fp);
     stmp:='';
     //пишем размер шрифта
    case fp.size of
      8: stmp := '[f5]'+trimleft(FMemo1.Lines[n]);
     10: stmp := '[f3]'+trimleft(FMemo1.Lines[n]);
     11: stmp := '[f6]'+trimleft(FMemo1.Lines[n]);
     12: stmp := '[f1]'+trimleft(FMemo1.Lines[n]);
     14: stmp := '[f4]'+trimleft(FMemo1.Lines[n]);
     16: stmp := '[f2]'+trimleft(FMemo1.Lines[n]);
    end;
    //выравнивание
    case fp.Justify of
    fjLeft :   stmp := '[fleft]'+stmp;
    ...
    //ищем переменные-услуги
    //while utf8pos(']',stmp)>0 do
    //begin
    //  usl :='';
    //  usl := utf8copy(stmp,utf8pos('[',stmp)+1,utf8pos(']',stmp)-utf8pos('[',stmp)-1);
    //  //ищем переменную в массиве услуг
    //  If trim(usl)<>'' then
    //  begin
    //    for i:=low(ar_uslug) to high(ar_uslug) do
    //     begin
    //      If trim(ar_uslug[i,1])=trim(usl) then
    //      begin
    //       svars := svars + usl +';';//добавляем переменную услугу в строку
    //       break;
    //      end;
    //     end;
    //  end;
    //
    //  stmp := utf8copy(stmp,utf8pos(']',stmp)+1,utf8length(stmp));
    //end;
    ZQuery1.SQL.add(stmp);
   end;
  //showmessage(svars);
  ...
  If not fl_dop_change then
   ...
   //************ если есть доп текст ****************
  If (trim(Combobox2.text)='') AND  (trim(FMemo2.Text)='') then exit;
   // Подключаемся к серверу
   If not(Connect2(Zconnection1, flagProfile)) then
     begin
      showmessagealt('Соединение с основным сервером потеряно !'+#13+'Проверьте соединение и/или'+#13+' обратитесь к администратору.');
 ...
 try
  ...
  ZQuery1.SQL.add(dop_id+','+QuotedStr(Fmemo2.Text)+','+inttostr(id_user)+',now(),0);');
  ZQuery1.open;
...
begin
  FormTT.FMemo1.Append('&Конец чека');
  FormTT.KMemo1.Blocks.AddTextBlock('&Конец чека');
end;
...
begin
  ...
  for n:=0 to dl-1 do
    begin
         ...
         FMemo1.SetFocus;
         FMemo1.GetFormat(fp);
         s:=s+'X:='+inttostr(n)+' Caret X='+inttostr(FMemo1.CaretPos.x)+' len_select='+inttostr(FMemo1.SelLength)+' fp.name='+fp.name+' text='+FMemo1.SelText+#13;
    end;
  FMemo1.SetFocus;
...
procedure TformTT.KMemo1Click(Sender: TObject);
begin
  formTT.StatusBar1.Canvas.Clear;
  formTT.StatusBar1.Canvas.Font.Size:=10;
  formTT.StatusBar1.Canvas.TextOut(5,0,
   //'Текущая позиция. Символ:'+inttostr(formTT.KMemo1.CaretPos.x)
    ' Строка:'+inttostr(formTT.kMemo1.Blocks.Lines.Count)
    +' Шрифт='+inttostr(formTT.KMemo1.SelectionTextStyle.Font.Size));
end;
...
begin
 ...
 // Ставим в начало
    //pos.x:=0;
    //pos.y:=FMemo1.CaretPos.Y;
    FMemo1.SetFocus;
    //FMemo1.CaretPos:=pos;
    //FMemo1.SelStart := FMemo1.SelStart-FMemo1.CaretPos.X;
   //выделяем всю строку
   //FMemo1.SelLength := UTF8Length(FMemo1.Lines[FMemo1.CaretPos.Y]);
     FMemo1.GetFormat(fp);
  end;
     //если в текст шаблона
  If PageControl1.ActivePageIndex=1 then
...
begin
  ...
  If PageControl1.ActivePageIndex=0 then
  begin
     //KMemo1.Select(KMemo1.Blocks.LineStartIndex[KMemo1.Blocks.IndexToLineIndex(KMemo1.CaretPos)]
           //,KMemo1.Blocks.LineSize[KMemo1.Blocks.IndexToLineIndex(KMemo1.CaretPos)],false);
      KMemo1.ExecuteCommand(ecLineStart);
      KMemo1.ExecuteCommand(ecSelLineStart);
      KMemo1.ExecuteCommand(ecSelLineEnd);


 //шрифт № 6
if lffind and (KMemo1.SelectionTextStyle.Font.Size=10) then
begin
   try
    TextStyle.Font.Style := [fsItalic];
    TextStyle.Font.Size:= 11;
...
//шрифт № 1
  if lffind and (KMemo1.SelectionTextStyle.Font.Size=11) then
  begin
    try
    //TextStyle.Font.Style := [fsItalic];
...
//шрифт № 4
  if lffind and (KMemo1.SelectionTextStyle.Font.Size=12) then
  begin
    try
...
//шрифт № 2
  if lffind and (KMemo1.SelectionTextStyle.Font.Size=14) then
begin
   try
    TextStyle.Font.Style := [fsBold];
...
//шрифт № 5
  if lffind and (KMemo1.SelectionTextStyle.Font.Size=16) then
begin
  try
  ...
  //шрифт № 3
  if lffind and (KMemo1.SelectionTextStyle.Font.Size=8) then
 begin
   try
...
// Ставим в конец
   //pos.x:=UTF8Length(FMemo1.Lines[FMemo1.CaretPos.Y])-1;
   //pos.y:=FMemo1.CaretPos.Y;
   //FMemo1.CaretPos:=pos;
 end;
end;
...
procedure TformTT.ToBParaLeftClick(Sender: TObject);
begin
  // Делаем выравнивание СЛЕВА
  StrAlign('left');
end;
