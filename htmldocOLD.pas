...
uses
//  Classes, SysUtils,ExtCtrls,htmlproc,unix,StdCtrls, LCLIntf, LCLType,Controls,FileUtil, LMessages, Messages;
...
implementation
uses
 mainopp, report_main, report_vibor, shedule_edit;
...
//--------------------------------------------------------------------------------------------
//------------------//Пересечение календарных планов выхода перевозчиков на расписании-------
//--------------------------------------------------------------------------------------------
procedure Report_SheduleCost();
var
 n,x:integer;
begin
 // Начало HTML
 if StartHTML(ExtractFilePath(Application.ExeName)+'shedule_cost.html')=false then exit;
 SetHTMLString('Пересечение календарных планов выхода перевозчиков на расписании.',3,3,4,'000000');

 SetHTMLString('',3,2,2,'',0,0);
 SetHTMLString('',3,2,2,'',0,0);
 // Таблица - Начало
...
begin
  ...
  SetHTMLString('Пересечение календарных планов выхода перевозчиков на расписании.',3,3,4,'',0,0);
...
//--------------------------------------------------------------------------------------------
//---------------------- Цена билетов по основному тарифу расписания--------------------------
//--------------------------------------------------------------------------------------------
procedure Report_shedule_tarif();
var
  sCol : string;
  n,m,i,k,num:integer;
  tt : array[2..7] of single;
  formir, an: byte;
  s,srow: string;
begin
       // Начало HTML
      //if StartHTML(ExtractFilePath(Application.ExeName)+rep_name+'.html')=false then exit;
  for n:=low(ar_report) to high(ar_report) do
     begin
       s:=s+ar_report[n,0]+', '+ar_report[n,2]+ #13;
     end;
  //showmessagealt(inttostr(Length(ar_report))+#13+s);
    ...
    writeln(fileR,'&table|');
    StartTableHTML();
    //разбор заголовка таблицы
    DoColumns(rep_sCol);

  // Начало HTML
  //if StartHTML(ExtractFilePath(Application.ExeName)+'tarif_shedule.html')=false then exit;
  //SetHTMLString('Отчет о стоимости проезда на маршруте ',3,2,2,'',0,0);
  //SetHTMLString('№ '+form16.Edit3.Text+' - '+form16.Edit2.Text,3,2,2,'',0,0);
  //SetHTMLString('по расписанию № '+form16.Edit6.Text+' - '+form16.Edit5.Text,3,2,2,'',0,0);
  //SetHTMLString('',3,2,2,'',0,0);
  //SetHTMLString('',3,2,2,'',0,0);
  //SetHTMLString('Пункт начала отсчета: '+form16.StringGrid1.Cells[1,form16.StringGrid1.row],1,2,1,'',0,0);
  ...
  //имена столбцов
  //StartRowTableHTML();
  //CellsTableHTML('№',1,1,2,'',0,0);
  //CellsTableHTML('Остановочный пункт',1,1,2,'',0,0);
  //CellsTableHTML('Путь,км',1,1,2,'',0,0);
  //CellsTableHTML('Жесткий М3',1,1,2,'',0,0);
  //CellsTableHTML('Жесткий М2',1,1,2,'',0,0);
  //CellsTableHTML('Мягкий М2',1,1,2,'',0,0);
  //CellsTableHTML('Мягкий М3',1,1,2,'',0,0);
  //CellsTableHTML('Багаж',1,1,2,'',0,0);
  //EndRowTableHTML();
//обнуляем массив
 num:=0;
  for m:=low(tt) to high(tt) do
         begin
           tt[m] := 0.0;
         end;
  for n:=low(tarif_all) to high(tarif_all) do
    begin
      ...
      If num=0 then
          begin
            for m:=low(ar_report) to high(ar_report) do
              begin
                If ar_report[m,0]='rasp_station_kod' then break;
              end;
          If trim(ar_report[m,2])<>trim(tarif_all[n,0]) then continue;
          end;
      num := num+1;
        //проверяем на формирующийся
      formir := 1; //сбрасываем флаг - печатать обычным шрифтом
      for m:=n to high(m_sostav) do
        begin
         If trim(m_sostav[m,0])='' then continue;
         If trim(tarif_all[n,0])='' then continue;
         If (trim(m_sostav[m,0])=trim(tarif_all[n,0])) AND (trim(m_sostav[m,2])='1') then
             begin
               formir := 2; //печатать жирным шрифтом
               break;
             end;
         end;
       ...
       for m:=low(tt) to high(tt) do
         begin
           //Если не начало отсчета тарифа
           If num>1 then tt[m] := tt[m]+ strtofloat(tarif_all[n,m])
           else tt[m]:=0.0;
         end;

     //начинаем рисовать ячейки
     srow := '';
        StartRowTableHTML();
        //выбираем какие столбцы печатать
        ...
        begin
          ...
          an := 0;
          sCol := trim(ar_repcols[i,0]);
                  if scol='id_point' then an:=0;
                  if scol='name' then an:=1;
                  if scol='km' then an:=2;
                  if scol='hardm2' then an:=3;
                  if scol='hardm3' then an:=4;
                  if scol='softm2' then an:=5;
                  if scol='softm3' then an:=6;
                  if scol='bagazh' then an:=7;
                  if scol='tarif_hardm2' then an:=8;
                  if scol='tarif_hardm3' then an:=9;
                  if scol='tarif_softm2' then an:=10;
                  if scol='tarif_softm3' then an:=11;
                  if scol='tarif_bagazh' then an:=12;
                  if scol='id_kontr' then an:=13;
                  if scol='calculation_type' then an:=14;
        If (an>2) and (an<8) then
            begin
             srow := srow + floattostrF(tt[an],fffixed,10,2)+'|';
             CellsTableHTML(floattostrF(tt[an],fffixed,10,2),strtoint(ar_repcols[i,2]),strtoint(ar_repcols[i,3]),formir,'',0,0);
            end
        else
        begin
          srow := srow + tarif_all[n,an]+'|';
          CellsTableHTML(tarif_all[n,an],strtoint(ar_repcols[i,2]),strtoint(ar_repcols[i,3]),formir,'',0,0);
        end;
      ...
      //проверяем на формирующийся следующую остановку
      for m:=n+1 to high(m_sostav) do
        begin
         If trim(m_sostav[m,0])='' then continue;
         If trim(tarif_all[n,0])='' then continue;
         If (trim(m_sostav[m,0])=trim(tarif_all[n,0])) AND (trim(m_sostav[m,2])='1') then
             begin
               for k:=low(tt) to high(tt) do
                 begin
                   tt[k]:=0.0;
                 end;
               num:=1;
...
//------------------------------------Состав расписания---------------------------------------
//--------------------------------------------------------------------------------------------
procedure Report_sostav_shedule();
var
  n,m:integer;
begin
  // Начало HTML
  if StartHTML(ExtractFilePath(Application.ExeName)+'shedule.html')=false then exit;
  SetHTMLString('Документ РАСПИСАНИЕ',3,2,2,'FF0000');
  SetHTMLString('Маршрут № '+form16.Edit3.Text+'- '+form16.Edit2.Text,3,2,2,'FF0000');
  SetHTMLString('Расписание № '+form16.Edit6.Text+'- '+form16.Edit5.Text,3,2,2,'080000');
  SetHTMLString('',3,2,2,'',0,0);
  SetHTMLString('',3,2,2,'',0,0);
  // Таблица - Начало
  StartTableHTML();
  for n:=0 to form16.StringGrid1.RowCount-1 do
    begin
      StartRowTableHTML();
       for m:=0 to 9 do
         begin
           if m=5 then CellsTableHTML(trim(form16.StringGrid1.Cells[m,n]),2,4,2,'',0,0) else CellsTableHTML(trim(form16.StringGrid1.Cells[m,n]),3,2,1,'',0,0);
         end;
      EndRowTableHTML();
    end;
  //Таблица - Конец
  EndTableHTML();
  EndHTML(); // Конец HTML (fileHTML:десткриптор файла)
  startbrowser(ExtractFilePath(Application.ExeName)+'shedule.html');
end;
//---------------------------------------------------------------------------------------------
...
begin
   ...
   begin
     ...
     If (k>j) AND (k<n) then
         ...
         sStr := sStr + trim(sTmp);
         SetHTMLString(Copy(sStr,j+1,length(sStr)),strtoint(Copy(sStr,j-3,1)),strtoint(Copy(sStr,j-2,1)),strtoint(Copy(sStr,j-1,1)),'',0,0);
         end
     else
       SetHTMLString(Copy(Shead,j+1,n-1-j),strtoint(Copy(Shead,j-3,1)),strtoint(Copy(Shead,j-2,1)),strtoint(Copy(Shead,j-1,1)),'',0,0);
...
begin
  ...
  SetHTMLString('РАЗМЕР шрифта = 1',3,1,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 2',3,2,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 3',3,3,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 4',3,4,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 5',3,5,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 6',3,6,1,'',0,0);
  SetHTMLString('РАЗМЕР шрифта = 7',3,7,1,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('ПОЗИЦИЯ ВЛЕВО',1,2,1,'',0,0);
  SetHTMLString('ПОЗИЦИЯ ВПРАВО',2,2,1,'',0,0);
  SetHTMLString('ПОЗИЦИЯ ЦЕНТР',3,2,1,'',0,0);
  SetHTMLString('ПОЗИЦИЯ ПО ШИРИНЕ',4,2,1,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('Нормальный ШРИФТ',3,2,1,'',0,0);
  SetHTMLString('Большой ШРИФТ',3,2,2,'',0,0);
  SetHTMLString('Наклонный ШРИФТ',3,2,3,'',0,0);
  SetHTMLString('Наклонный Большой ШРИФТ',3,2,4,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('',3,2,1,'',0,0);
  SetHTMLString('Цвет ШРИФТ',3,2,4,'',0,0);
  ...
  SetHTMLString('Автоматизированная система продажи билетов PLATFORMA AV',3,1,1,'',0,0);
  // Таблица - Начало
  StartTableHTML();
  // 1 строка 2 столбца
  StartRowTableHTML();
  ...
  CellsTableHTML('Строка=2 Столбец=2',1,3,2,'F00000');
