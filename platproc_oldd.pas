...
uses
  Classes, SysUtils, ZConnection, ZDataset, FileUtil, Controls, Graphics,
  Dialogs, ExtCtrls, StdCtrls,dbf,LConvEncoding,Grids, Buttons,Forms,IniPropStorage, ComCtrls,LCLProc,TypInfo,unix,libc, dateutils,strUtils;
...
begin
  ...
  If flwait then
   ...
   //не получилось подключиться с первого раза открываем окно
    ...
    for i:=1 to cNCon do
     begin
     If i>1 then Delay(2); //ожидаем 2 секунды
      try
         ZCon.connect;
      except
         continue;
...
begin
  ...
  //  ar1[k-1, 0] := copy(sStables, pos(',',sStables)+1, Length(sStables)-pos(',',sStables)-1);
    ...
    If pos(':',tmpS)>0 then
     ...
     sStables := copy(sStables, pos(',', sStables)+1, Length(sStables));
    //showmessage('Table : '+sTable+'  Col : '+sCol);//$
...
begin
   for N := 1 to Utf8Length(S) do
     begin
      //пробел просто переносим
      If UTF8Copy(S,n,1)=#32 then
       begin
        ss:=ss+#32;
        continue;
       end;
...
procedure fillarray;
 begin
  ...
  namemonth[7]:='Июля';
  namemonth[8]:='Августа';
  namemonth[9]:='Сентября';
  namemonth[10]:='Октября';
  namemonth[11]:='Ноября';
  namemonth[12]:='Декабря';
